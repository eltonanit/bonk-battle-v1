...
export default function ArmyDetailPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#000' }}>
      ...
      {/* HEADER */}
      <header className="sticky top-0 z-50 border-b border-white/10 backdrop-blur-md" style={{ backgroundColor: 'rgba(0,0,0,0.8)' }}>
        <div className="flex items-center gap-4 px-4 py-3">
          ...
          <div className="flex items-center gap-3 flex-1">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
              {army.image_url ? (
                <Image
                  src={army.image_url}
                  alt={army.name}
                ...
                <span className="text-sm">{army.icon || '⚔️'}</span>
              )}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-white">{army.name}</span>
                <span className="text-gray-500 text-sm">{ticker}</span>
              </div>
              <div className="text-gray-500 text-xs">
                {army.member_count} members · Level {army.level || 1}
              </div>
            </div>
          </div>

          {/* Members Icon */}
          <button className="w-8 h-8 flex items-center justify-center text-white hover:bg-white/10 rounded-full transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex">
          ...
          >
            Orders
            {activeTab === 'orders' && (
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-1 bg-green-500 rounded-full" />
            )}
          </button>
          <button
      ...
      <div className="sticky bottom-0 border-t border-white/10 p-3" style={{ backgroundColor: '#000' }}>
        <div className="flex items-center gap-3">
          {/* User Avatar */}
