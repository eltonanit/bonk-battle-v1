...
export function parseTokenLaunchAccount(accountInfo: AccountInfo<Buffer>): TokenLaunchAccount | null {
    try {
        ...
        return {creator, mint, tier, targetSol, deadline, solRaised, status, createdAt, graduatedAt, meteoraPool, totalBuyers, totalTokensSold, holdersThawed, name, symbol, uri, bump};
    } catch (error) {
        console.error('Error parsing TokenLaunch:', error);
        return null;
    }
}
...
export function parseBuyerRecordAccount(accountInfo: AccountInfo<Buffer>): BuyerRecordAccount | null {
    try {
        ...
        offset += 8;
        const bump = data.readUInt8(offset);
        return {launch, buyer, solSpent, tokensReceived, refundClaimed, isThawed, firstBuyTimestamp, lastBuyTimestamp, bump};
    } catch (error) {
        console.error('Error parsing BuyerRecord:', error);
        return null;
    }
}
