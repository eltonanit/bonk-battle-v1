...
pub const VIRTUAL_SUPPLY: u64 = 1_073_000_000_000_000;

/// Total supply reale (1 miliardo)
pub const REAL_SUPPLY: u64 = 1_000_000_000_000_000;

/// Token disponibili sulla bonding curve (793.1M = 79.31%)
pub const CURVE_TOKENS: u64 = 793_100_000_000_000;
...
// Meteora
pub const METEORA_LIQUIDITY_PERCENT: u8 = 85;
...
pub mod stonks_fan {
    ...
    /// Batch thaw holders
    pub fn batch_thaw_holders<'info>(
        ctx: Context<'_, '_, '_, 'info, BatchThawHolders<'info>>,
        holder_count: u8,
    ) -> Result<()> {
        {
        ...
        require!(holder_count > 0 && holder_count as usize <= MAX_THAW_PER_TX, ErrorCode::InvalidBatchSize);
        
        let remaining_accounts = &ctx.remaining_accounts;
        require!(remaining_accounts.len() == holder_count as usize * 2, ErrorCode::InvalidAccountCount);
        
        let launch_key = ctx.accounts.token_launch.key();
        let launch_mint = ctx.accounts.token_launch.mint;
        let launch_bump = ctx.accounts.token_launch.bump;
        ...
        for i in 0..holder_count as usize {
            let buyer_record_info = &remaining_accounts[i * 2];
            let token_account_info = &remaining_accounts[i * 2 + 1];
            
            let mut buyer_record_data: &[u8] = &buyer_record_info.try_borrow_data()?;
            let buyer_record = BuyerRecord::try_deserialize(&mut buyer_record_data)?;
            
            require!(buyer_record.launch == launch_key, ErrorCode::InvalidLaunch);
            ...
            require!(
                token_account.owner == buyer_record.buyer,
                ErrorCode::InvalidTokenAccount
            );
            require!(
                token_account.mint == launch_mint,
            ...
            msg!("✅ Thawed holder {}/{}", i + 1, holder_count);
        }
        
        let launch = &mut ctx.accounts.token_launch;
        launch.holders_thawed = launch
            .holders_thawed
            .checked_add(holder_count as u32)
            .ok_or(ErrorCode::MathOverflow)?;
        
        emit!(HoldersThawed {
            mint: launch.mint,
            count: holder_count,
            total_thawed: launch.holders_thawed,
            total_holders: launch.total_buyers,
        });
        
        msg!("📊 Progress: {}/{} holders thawed", launch.holders_thawed, launch.total_buyers);
    ...
    pub fn finalize_graduation(ctx: Context<FinalizeGraduation>) -> Result<()> {
        ...
        require!(launch.status == LaunchStatus::ReadyToGraduate, ErrorCode::NotReadyToGraduate);
        require!(
            launch.holders_thawed >= launch.total_buyers,
            ErrorCode::HoldersNotThawed
        );
        require!(launch.sol_raised >= launch.target_sol, ErrorCode::TargetNotReached);
        ...
        let total_sol = **launch.to_account_info().try_borrow_lamports()?;
        let liquidity_sol = total_sol
            .checked_mul(METEORA_LIQUIDITY_PERCENT as u64)
            .ok_or(ErrorCode::MathOverflow)?
            .checked_div(100)
            .ok_or(ErrorCode::MathOverflow)?;
        
        let tokens_for_liquidity = REAL_SUPPLY
            .checked_sub(launch.total_tokens_sold)
            .ok_or(ErrorCode::MathOverflow)?;
        ...
        **ctx.accounts.pool_sol_vault.try_borrow_mut_lamports()? += liquidity_sol;
        ...
        msg!("🔧 Meteora pool creation placeholder - integrate Meteora SDK");
        ...
        emit!(TokenGraduated {
            ...
            liquidity_provided: liquidity_sol,
            tokens_for_liquidity,
...
impl TokenLaunch {
    ...
    pub const SIZE: usize = 32 + 32 + 1 + 8 + 16 + 8 + 8 + 8 + 1 + 8 + 9 + 33 + 4 + 8 + 4 + 36 + 14 + 204 + 1;
}
...
#[derive(Accounts)]
pub struct BatchThawHolders<'info> {
    #[account(mut)]
    ...
    #[account(constraint = mint.key() == token_launch.mint @ ErrorCode::InvalidMint)]
    pub mint: Account<'info, Mint>,
...
#[event]
pub struct HoldersThawed {
    pub mint: Pubkey,
    pub count: u8,
    pub total_thawed: u32,
...
fn get_tier_params(tier: u8) -> Result<(u64, u128, u64)> {
    match tier {
        1 => Ok((
            // ⚠️ TESTING: Virtual SOL ridotto proporzionalmente per target 1 SOL
            73_170_000,                              // 0.07317 SOL (scaled down)
            78_419_000_000_000_000_000,              // k scaled (maintains curve shape)
            TIER_1_TARGET_SOL,                       // 1 SOL
        )),
        2 => Ok((
            186_410_000_000,                         // 186.41 SOL
            200_017_930_000_000_000_000_000,        // k = 200T
            TIER_2_TARGET_SOL,                       // 2,551 SOL
        )),
        3 => Ok((
            1_864_980_000_000,                       // 1,864.98 SOL
            2_001_123_540_000_000_000_000_000,      // k = 2T
            TIER_3_TARGET_SOL,                       // 25,510 SOL
        )),
        4 => Ok((
            18_649_790_000_000,                      // 18,649.79 SOL
            20_011_214_670_000_000_000_000_000,     // k = 20,000T
